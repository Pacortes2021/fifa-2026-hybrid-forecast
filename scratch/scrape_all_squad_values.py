...
LEAGUES = {
    "mex": {
        ...
        "out_csv": ROOT / "mex" / "data" / "squad_values_historical.csv",
        "manual": {
    ...
    "bra": {
        ...
        "out_csv": ROOT / "bra" / "data" / "squad_values_historical.csv",
        "manual": {
            ...
            "Grêmio Novorizontino": "Mirassol" # Fallback/Check
        }
    },
    "chile": {
        "tm_id": "CLPD",
        "espn_partidos": ROOT / "chile" / "data" / "partidos.csv",
        "out_csv": ROOT / "chile" / "data" / "squad_values_historical.csv",
        "manual": {
    ...
    "eng": {
        ...
        "out_csv": ROOT / "eng" / "data" / "squad_values_historical.csv",
        "manual": {
...
def scrape_all(only=None, seasons=None):
    headers = {
    ...
    for league_key, info in LEAGUES.items():
        if only and league_key not in only:
            continue
        print(f"\n=======================================================")
        print(f"Scrapeando valores de plantilla para {league_key.upper()} ({info['tm_id']})")
        print(f"=======================================================")
        
        # Obtener nombres de ESPN en partidos
        ...
        for s in seasons:
            ...
            try:
                ...
                for row in rows:
                    ...
                    count += 1
                print(f"    Completado: {count} equipos procesados para {s}")
                time.sleep(2)
                
            except Exception as ex:
                print(f"    Error scraping season {s}: {ex}")
                
        if filas_liga:
            df = pd.DataFrame(filas_liga)
            ...
            print(f"  ✓ Guardado {info['out_csv']} con {len(df)} registros.")
        else:
            print(f"  ✗ No se recopilaron datos para {league_key.upper()}")

if __name__ == "__main__":
    import sys
    only = sys.argv[1].split(",") if len(sys.argv) > 1 else None
    seasons = [int(x) for x in sys.argv[2].split(",")] if len(sys.argv) > 2 else None
    scrape_all(only=only, seasons=seasons)
