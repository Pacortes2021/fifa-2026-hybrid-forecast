...
DESCIENDEN = 1  # En Escocia desciende el 12º directo (11º juega playoff)
CUPOS_COPA = 5  # Champions (1-2), Europa League (3), Conference (4-5)

SQUAD_VALUES = {
    "Celtic": 140.0, "Rangers": 110.0, "Heart of Midlothian": 25.0,
    "Aberdeen": 20.0, "Hibernian": 18.0, "Dundee United": 12.0,
    "Kilmarnock": 12.0, "Dundee": 10.0, "Motherwell": 10.0,
    "St Mirren": 11.0, "St Johnstone": 9.0, "Ross County": 8.0,
    "Livingston": 7.0, "Falkirk": 7.0, "Hamilton Academical": 5.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Scottish Premiership
HOME_ADV = 50.0    # Ventaja de local típica en Escocia

COORDS_SCOTLAND = {
    "Celtic": (55.8497, -4.2055), "Rangers": (55.8532, -4.3093),
    "Heart of Midlothian": (55.9391, -3.2325), "Hibernian": (55.9617, -3.1656),
    "Aberdeen": (57.1593, -2.0886), "Dundee United": (56.4747, -2.9719),
    "Dundee": (56.4747, -2.9733), "Kilmarnock": (55.6044, -4.5083),
    "Motherwell": (55.7797, -3.9806), "St Mirren": (55.8569, -4.4361),
    "St Johnstone": (56.4094, -3.4739), "Ross County": (57.5958, -4.4189),
    "Livingston": (55.8864, -3.5228), "Falkirk": (56.0050, -3.7533),
    "Hamilton Academical": (55.7822, -4.0583)
}

ALTITUDES_SCOTLAND = {
    "Celtic": 25, "Rangers": 10, "Heart of Midlothian": 55, "Hibernian": 30,
    "Aberdeen": 15, "Dundee United": 75, "Dundee": 75, "Kilmarnock": 40,
    "Motherwell": 85, "St Mirren": 15, "St Johnstone": 45, "Ross County": 10,
    "Livingston": 120, "Falkirk": 15, "Hamilton Academical": 60
}
...
def get_distance_km(local, visita):
    if local in COORDS_SCOTLAND and visita in COORDS_SCOTLAND:
        c1 = COORDS_SCOTLAND[local]; c2 = COORDS_SCOTLAND[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 85.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_SCOTLAND.get(local, 35)
    av = ALTITUDES_SCOTLAND.get(visita, 35)
    return float(al - av)
...
def get_squad_value(equipo, temporada):
    ...
    return float(SQUAD_VALUES.get(equipo, 15.0))
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 26.0, "foreigners": 12.0, "pct_foreigners": 0.42,
        "squad_value": float(SQUAD_VALUES.get(equipo, 15.0)), "stadium_capacity": 15000.0,
...
def cargar():
    ...
    print("Entrenando modelo de Scottish Premiership (Stacking + Poisson)... esto toma ~10s")
    ...
    print(f"Metricas SCO Test>=2025: LASSO={metricas['lasso']} RF={metricas['rf']} Stacking={metricas['stacking']}")
