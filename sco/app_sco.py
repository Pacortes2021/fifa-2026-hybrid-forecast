"""
Aplicación Streamlit para la simulación y predicciones de la Scottish Premiership (Escocia).
...
# Insertar el directorio sco en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#002B7F,#0A192F,#D4AF37); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#002B7F;
...
# Emojis e identidades visuales de los clubes escoceses
TEAM_DETAILS = {
    "Celtic": {"flag": "🟢⚪", "color": "#018749"},
    "Rangers": {"flag": "🔵🔴", "color": "#1b458f"},
    "Heart of Midlothian": {"flag": "🟤⚪", "color": "#800000"},
    "Aberdeen": {"flag": "🔴⚪", "color": "#e31b23"},
    "Hibernian": {"flag": "🟢⚪", "color": "#005837"},
    "Dundee United": {"flag": "🟠⚫", "color": "#f37023"},
    "Kilmarnock": {"flag": "🔵⚪", "color": "#122a5e"},
    "Dundee": {"flag": "🔵⚪", "color": "#082567"},
    "Motherwell": {"flag": "🟡🟤", "color": "#ffc72c"},
    "St Mirren": {"flag": "⚫⚪", "color": "#000000"},
    "St Johnstone": {"flag": "🔵⚪", "color": "#004890"},
    "Ross County": {"flag": "🔵⚪", "color": "#00205b"},
    "Livingston": {"flag": "🟡⚫", "color": "#fdb913"},
    "Falkirk": {"flag": "🔵⚪", "color": "#00205b"},
    "Hamilton Academical": {"flag": "🔴⚪", "color": "#e31b23"}
}
...
def run_app():
    ...
    M = get_motor()
    if M is None:
        st.error("No se encontraron datos históricos de la Scottish Premiership. Ejecuta la recolección primero.")
        return

    tracker = M["tracker"]
    equipos_data = M.get("equipos", {})

    st.markdown('<div class="main-title">🏴󠁧󠁢󠁳󠁣󠁴󠁿 Scottish Premiership Predictor</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fixture_disp.empty:
            st.markdown("#### ⚡ Próximos Partidos Oficiales de Scottish Premiership")
    ...
    with tab2:
        ...
        with col_t2:
            ...
            df_mc = simular_liga(M, "sim_key_sco", modelo_tipo)
        ...
        st.info("ℹ️ **Reglas de Scottish Premiership**: Puestos 1-2 clasifican a UEFA Champions League, 3 a Europa League, 4-5 a Conference League. El último puesto (12º) desciende directo al Championship y el 11º disputa playoff de permanencia.")
    ...
    with tab3:
        ...
        if rf_step and hasattr(rf_step, "feature_importances_"):
            ...
            ax.barh(df_imp["Variable"][::-1], df_imp["Importancia"][::-1], color="#002B7F")
