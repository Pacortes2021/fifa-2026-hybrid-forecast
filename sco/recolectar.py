"""
Recolector de la Scottish Premiership (Escocia) desde la API pública de ESPN (liga "sco.1").
Convierte la fecha UTC de ESPN a la hora local escocesa ("Europe/London").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/sco.1/scoreboard"
...
NORM_MAP = {
    ...
    "Hamilton Accies": "Hamilton Academical",
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Escocia (Londres / GMT / BST)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/London").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Escocia.")
        return
    ...
    print(f"Guardado {out_p} ({len(df_jugados)} partidos jugados de Escocia)")
