"""
Recolector de la Eredivisie de Países Bajos desde la API pública de ESPN (liga "ned.1").
Convierte la fecha UTC de ESPN a la hora local neerlandesa ("Europe/Amsterdam").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/ned.1/scoreboard"
TEAMS_URL = "https://site.api.espn.com/apis/site/v2/sports/soccer/ned.1/teams"
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Países Bajos (Ámsterdam)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Amsterdam").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Países Bajos.")
        return
    ...
    print(f"Guardado Países Bajos: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Países Bajos: fixture.csv ({len(fixture)} por jugar)")
