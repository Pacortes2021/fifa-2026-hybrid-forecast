"""
Aplicación Streamlit para la simulación y predicciones de la Eredivisie (Países Bajos).
...
# Insertar el directorio ned en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#FF4F00,#21468B); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#FF4F00;
    border-bottom:2px solid #e2e8f0; padding-bottom:0.4rem; margin-bottom:0.8rem; }
.sec-title { font-size:1.6rem; font-weight:800; color:#21468B; margin:0.8rem 0 0.6rem 0; }
...
# Emojis e identidades visuales de los clubes neerlandeses
TEAM_DETAILS = {
    "PSV": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Feyenoord": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Ajax": {"flag": "⚪🔴", "color": "#DF1B27"},
    "AZ Alkmaar": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Twente": {"flag": "🔴⚪", "color": "#F31522"},
    "Utrecht": {"flag": "🔴⚪", "color": "#F31522"},
    "NEC": {"flag": "🔴🟢⚫", "color": "#ef2f24"},
    "Heerenveen": {"flag": "🔵⚪❤️", "color": "#003eff"},
    "Sparta Rotterdam": {"flag": "🔴⚪", "color": "#F31522"},
    "Go Ahead Eagles": {"flag": "🟡🔴", "color": "#F80017"},
    "Groningen": {"flag": "🟢⚪", "color": "#30565c"},
    "Fortuna Sittard": {"flag": "🟡🟢", "color": "#FCEE33"},
    "PEC Zwolle": {"flag": "🔵⚪", "color": "#0000d4"},
    "Heracles Almelo": {"flag": "⚫⚪", "color": "#000000"},
    "NAC Breda": {"flag": "🟡⚫", "color": "#FCEE33"},
    "Willem II": {"flag": "🔴⚪🔵", "color": "#1a316b"},
    "Almere City": {"flag": "🔴⚫", "color": "#C60000"},
    "Vitesse": {"flag": "🟡⚫", "color": "#FCEE33"},
    "Excelsior": {"flag": "🔴⚫", "color": "#b41226"},
    "FC Volendam": {"flag": "🟠", "color": "#F7AA25"},
    "RKC Waalwijk": {"flag": "🟡🔵", "color": "#0c57f2"},
    "ADO Den Haag": {"flag": "🟡🟢", "color": "#307B64"},
    "SC Cambuur": {"flag": "🟡🔵", "color": "#FCEE33"},
    "FC Emmen": {"flag": "🔴⚪", "color": "#F80017"},
    "De Graafschap": {"flag": "🔵⚪", "color": "#0000d4"},
    "Telstar": {"flag": "⚪🔵", "color": "#C60000"}
}
...
def run_app():
    ...
    st.markdown('<div class="main-title">🇳🇱 Portal de Predicción Eredivisie</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="main-subtitle">Modelo activo: <b>{nombre_modelo}</b> — LASSO + RF + XGBoost + Simulación de Schaal, Champions y Descenso</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_ned():
                sel = st.session_state.get("sel_fix_ned")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_ned"] = l_sel
                        st.session_state["sel_b_ned"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_ned",
                on_change=_on_fixture_change_ned,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            def_a = "PSV" if "PSV" in opciones else opciones[0]
            a = st.selectbox("Equipo Local", opciones, index=opciones.index(def_a), key="sel_a_ned", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            def_b = "Feyenoord" if "Feyenoord" in opciones else ("Ajax" if "Ajax" in opciones else opciones[1])
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index(def_b), key="sel_b_ned", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            with st.expander("🔬 Ver Radiografía Táctica y Métricas Comparativas", expanded=True):
                ...
                st.markdown(
                    ...
                    f"<span style='color:#FF4F00;'>🏠 {a}</span>"
                    f"<span style='color:#6b7280;font-size:.8rem;'>Variable</span>"
                    f"<span style='color:#21468B;'>✈️ {b}</span></div>", unsafe_allow_html=True)
        ...
        with col_proj:
            ...
            st.dataframe(
                df_proy_visual.style.format({
                    ...
                    "🇪🇺 P(Copas)": "{:.1%}",
                    "🔻 P(Descenso)": "{:.1%}"
                }).background_gradient(subset=["🏆 P(Schaal/Campeón)"], cmap="Greens")
    ...
    with tab3:
        ...
        with col_g:
            ...
            ax.barh(top_n["Variable"][::-1], top_n[col_val_name][::-1], color="#FF4F00")
        ...
        if df_val is None or len(df_val) == 0:
            ...
            with c_plot:
                if met["n"] >= 3:
                    ...
                    ax.plot(evol["partidos"], evol["logloss"], "o-", color="#FF4F00", label="Modelo (acumulado)")
                    ax.axhline(met["logloss_base"], color="#21468B", ls="--", lw=1.5, label="Baseline")
