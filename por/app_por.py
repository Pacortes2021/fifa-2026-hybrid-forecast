"""
Aplicación Streamlit para la simulación y predicciones de la Primeira Liga (Portugal).
...
# Insertar el directorio por en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#006600,#FF0000); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#006600;
    border-bottom:2px solid #e2e8f0; padding-bottom:0.4rem; margin-bottom:0.8rem; }
.sec-title { font-size:1.6rem; font-weight:800; color:#006600; margin:0.8rem 0 0.6rem 0; }
...
# Emojis e identidades visuales de los clubes portugueses
TEAM_DETAILS = {
    "Sporting CP": {"flag": "🟢⚪", "color": "#008127"},
    "Benfica": {"flag": "🔴⚪", "color": "#ca281d"},
    "Porto": {"flag": "🔵⚪", "color": "#0000dd"},
    "Braga": {"flag": "🔴⚪", "color": "#de1f26"},
    "Vitoria de Guimaraes": {"flag": "⚪⚫", "color": "#000000"},
    "Famalicao": {"flag": "🔵⚪", "color": "#183760"},
    "Arouca": {"flag": "🟡🔵", "color": "#ffea01"},
    "Rio Ave": {"flag": "🟢⚪", "color": "#3b8649"},
    "Gil Vicente": {"flag": "🔴🔵", "color": "#de1f26"},
    "Moreirense": {"flag": "🟢⚪", "color": "#288A00"},
    "Estoril": {"flag": "🟡🔵", "color": "#ffea01"},
    "Casa Pia": {"flag": "⚫🔴", "color": "#000000"},
    "Boavista": {"flag": "🏁", "color": "#1a1a1a"},
    "Santa Clara": {"flag": "🔴⚪", "color": "#C60000"},
    "SC Farense": {"flag": "⚫⚪", "color": "#000000"},
    "Estrela": {"flag": "🟢🔴⚪", "color": "#3B8132"},
    "AVS": {"flag": "🔴⚪", "color": "#C60000"},
    "Nacional": {"flag": "⚫⚪", "color": "#000000"},
    "Portimonense": {"flag": "⚫⚪", "color": "#000000"},
    "Vizela": {"flag": "🔵⚪", "color": "#293dc2"},
    "GD Chaves": {"flag": "🔴🔵", "color": "#288A00"},
    "Paços de Ferreira": {"flag": "🟡🟢", "color": "#ffea01"},
    "Tondela": {"flag": "🟡🟢", "color": "#ffea01"},
    "Académico de Viseu": {"flag": "⚫⚪", "color": "#000000"},
    "Alverca": {"flag": "🔵🔴", "color": "#0047AB"},
    "Maritimo": {"flag": "🟢🔴", "color": "#008222"},
    "Belenenses": {"flag": "🔵⚪", "color": "#293dc2"}
}
...
def run_app():
    ...
    st.markdown('<div class="main-title">🇵🇹 Portal de Predicción Primeira Liga</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="main-subtitle">Modelo activo: <b>{nombre_modelo}</b> — LASSO + RF + XGBoost + Simulación de Campeón, Champions y Descenso</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_por():
                sel = st.session_state.get("sel_fix_por")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_por"] = l_sel
                        st.session_state["sel_b_por"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_por",
                on_change=_on_fixture_change_por,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            def_a = "Sporting CP" if "Sporting CP" in opciones else opciones[0]
            a = st.selectbox("Equipo Local", opciones, index=opciones.index(def_a), key="sel_a_por", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            def_b = "Benfica" if "Benfica" in opciones else ("Porto" if "Porto" in opciones else opciones[1])
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index(def_b), key="sel_b_por", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            with st.expander("🔬 Ver Radiografía Táctica y Métricas Comparativas", expanded=True):
                ...
                st.markdown(
                    ...
                    f"<span style='color:#006600;'>🏠 {a}</span>"
                    f"<span style='color:#6b7280;font-size:.8rem;'>Variable</span>"
                    f"<span style='color:#FF0000;'>✈️ {b}</span></div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with col_g:
            ...
            ax.barh(top_n["Variable"][::-1], top_n[col_val_name][::-1], color="#006600")
        ...
        if df_val is None or len(df_val) == 0:
            ...
            with c_plot:
                if met["n"] >= 3:
                    ...
                    ax.plot(evol["partidos"], evol["logloss"], "o-", color="#006600", label="Modelo (acumulado)")
                    ax.axhline(met["logloss_base"], color="#FF0000", ls="--", lw=1.5, label="Baseline")
