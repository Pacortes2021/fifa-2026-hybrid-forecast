...
SQUAD_VALUES = {
    "Sporting CP": 450.0, "Benfica": 360.0, "Porto": 330.0, "Braga": 135.0,
    "Vitoria de Guimaraes": 55.0, "Famalicao": 45.0, "Arouca": 35.0, "Rio Ave": 30.0,
    "Gil Vicente": 28.0, "Moreirense": 28.0, "Estoril": 25.0, "Casa Pia": 22.0,
    "Boavista": 20.0, "Santa Clara": 20.0, "SC Farense": 18.0, "Estrela": 18.0,
    "AVS": 16.0, "Nacional": 15.0, "Portimonense": 15.0, "Vizela": 14.0,
    "GD Chaves": 12.0, "Paços de Ferreira": 12.0, "Tondela": 10.0,
    "Académico de Viseu": 8.0, "Alverca": 8.0, "Maritimo": 8.0, "Belenenses": 6.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Primeira Liga
HOME_ADV = 53.0    # Ventaja de local típica en Portugal

COORDS_PORTUGAL = {
    "Sporting CP": (38.7611, -9.1606), "Benfica": (38.7528, -9.1847),
    "Porto": (41.1617, -8.5836), "Braga": (41.5628, -8.4300),
    "Vitoria de Guimaraes": (41.4450, -8.3014), "Famalicao": (41.4139, -8.5208),
    "Arouca": (40.9328, -8.2458), "Rio Ave": (41.3653, -8.7436),
    "Gil Vicente": (41.5367, -8.6189), "Moreirense": (41.3814, -8.3411),
    "Estoril": (38.7103, -9.3972), "Casa Pia": (38.7369, -9.1889),
    "Boavista": (41.1622, -8.6433), "Santa Clara": (37.7444, -25.6667),
    "SC Farense": (37.0211, -7.9333), "Estrela": (38.7547, -9.2369),
    "AVS": (41.3639, -8.4111), "Nacional": (32.6583, -16.8917),
    "Portimonense": (37.1364, -8.5383), "Vizela": (41.3789, -8.3072),
    "GD Chaves": (41.7436, -7.4667), "Paços de Ferreira": (41.2789, -8.3844),
    "Tondela": (40.5186, -8.0825), "Académico de Viseu": (40.6575, -7.9142),
    "Alverca": (38.8986, -9.0397), "Maritimo": (32.6500, -16.9300),
    "Belenenses": (38.7028, -9.2069)
}

ALTITUDES_PORTUGAL = {
    "Sporting CP": 70, "Benfica": 65, "Porto": 100, "Braga": 170,
    "Vitoria de Guimaraes": 215, "Famalicao": 90, "Arouca": 250, "Rio Ave": 20,
    "Gil Vicente": 40, "Moreirense": 175, "Estoril": 45, "Casa Pia": 80,
    "Boavista": 95, "Santa Clara": 50, "SC Farense": 10, "Estrela": 120,
    "AVS": 110, "Nacional": 580, "Portimonense": 15, "Vizela": 160,
    "GD Chaves": 360, "Paços de Ferreira": 320, "Tondela": 310, "Académico de Viseu": 450,
    "Alverca": 25, "Maritimo": 60, "Belenenses": 40
}
...
def get_distance_km(local, visita):
    if local in COORDS_PORTUGAL and visita in COORDS_PORTUGAL:
        c1 = COORDS_PORTUGAL[local]; c2 = COORDS_PORTUGAL[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 280.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_PORTUGAL.get(local, 100)
    av = ALTITUDES_PORTUGAL.get(visita, 100)
    return float(al - av)
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 25.5, "foreigners": 15.0, "pct_foreigners": 0.55,
        "squad_value": float(SQUAD_VALUES.get(equipo, 30.0)), "stadium_capacity": 20000.0,
        "avg_attendance": 12000.0, "stadium_occupation": 0.65
    }
...
def cargar_y_entrenar():
    ...
    print(f"Metricas POR Test>=2025: LASSO={m_l} RF={m_r} Stacking={m_s}")
...
def cargar():
    ...
    print("Entrenando modelo de Primeira Liga (Stacking + Poisson)... esto toma ~10s")
    M = cargar_y_entrenar()
    try:
