"""
Recolector de la Primeira Liga de Portugal desde la API pública de ESPN (liga "por.1").
Convierte la fecha UTC de ESPN a la hora local portuguesa ("Europe/Lisbon").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/por.1/scoreboard"
TEAMS_URL = "https://site.api.espn.com/apis/site/v2/sports/soccer/por.1/teams"
TEMPORADAS = list(range(2021, int(datetime.now().year) + 1))

NORM_MAP = {
    "Vitória de Guimaraes": "Vitoria de Guimaraes",
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Ignorar partidos no jugados o suspendidos
            ...
            # Convertir hora UTC a la zona horaria local de Portugal (Lisboa)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Lisbon").tz_localize(None)
...
def _canonizar_por_id(df):
    ...
    nombres distintos entre temporadas, todo su historial queda bajo el nombre más reciente."""
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Portugal.")
        return
    ...
    print(f"Guardado Portugal: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Portugal: fixture.csv ({len(fixture)} por jugar)")
