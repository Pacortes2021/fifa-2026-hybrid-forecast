...
def run_app():
    ...
    # ── Tabla de métricas por modelo (sidebar) ───────────────────────────
    ...
    st.markdown(f'<div class="main-subtitle">Modelo activo: <b>{nombre_modelo}</b> — LASSO + RF + Simulación de Liguilla y Play-In</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_mex():
                sel = st.session_state.get("sel_fix_mex")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_mex"] = l_sel
                        st.session_state["sel_b_mex"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_mex",
                on_change=_on_fixture_change_mex,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            a = st.selectbox("Equipo Local", opciones, index=opciones.index("América") if "América" in opciones else 0, key="sel_a_mex", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index("Guadalajara") if "Guadalajara" in opciones else (1 if len(opciones) > 1 else 0), key="sel_b_mex", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            # Calcular grilla de goles con el modelo seleccionado
            ...
            partidos_df_mex = M["partidos"]
            mask_h2h = ((partidos_df_mex["local"] == a) & (partidos_df_mex["visita"] == b)) | \
                       ((partidos_df_mex["local"] == b) & (partidos_df_mex["visita"] == a))
            df_h2h = partidos_df_mex[mask_h2h].copy()
    ...
    with tab2:
        ...
        df_proy = simular_liga(M, last_key, modelo)
        ...
        df_proy_visual = df_proy_visual[["Escudo", "Equipo", "Puntos esperados", "P_directo_QF", "P_Liguilla_total", "P_campeon"]]
