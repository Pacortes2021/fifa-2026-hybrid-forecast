"""
Aplicación Streamlit para la simulación y predicciones del Brasileirão (Serie A de Brasil).
Visualizaciones premium de Versus, Mercados, Tabla de Posiciones y Simulación de Copas y Descenso.
"""
import os
import sys
import numpy as np
...
# Insertar el directorio bra en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#00a859,#ffcc00); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#00a859;
    border-bottom:2px solid #e2e8f0; padding-bottom:0.4rem; margin-bottom:0.8rem; }
.sec-title { font-size:1.6rem; font-weight:800; color:#00a859; margin:0.8rem 0 0.6rem 0; }
...
# Emojis e identidades visuales de los clubes brasileños
TEAM_DETAILS = {
    "Palmeiras": {"flag": "🟢", "color": "#006437"},
    "Flamengo": {"flag": "🔴", "color": "#c41221"},
    "Atlético-MG": {"flag": "⚫", "color": "#000000"},
    "São Paulo": {"flag": "🔴", "color": "#e30613"},
    "Botafogo": {"flag": "🌟", "color": "#000000"},
    "Corinthians": {"flag": "🦅", "color": "#000000"},
    "Fluminense": {"flag": "🟢", "color": "#8b0000"},
    "Internacional": {"flag": "🔴", "color": "#dd0000"},
    "Grêmio": {"flag": "🔵", "color": "#0d80c6"},
    "Athletico-PR": {"flag": "🔴", "color": "#dd0000"},
    "Red Bull Bragantino": {"flag": "⚪", "color": "#d30a14"},
    "Bahia": {"flag": "🔵", "color": "#0056a3"},
    "Vasco da Gama": {"flag": "💢", "color": "#000000"},
    "Cruzeiro": {"flag": "🔵", "color": "#003399"},
    "Fortaleza": {"flag": "🔵", "color": "#005bb7"},
    "Santos": {"flag": "🐳", "color": "#000000"},
    "Vitória": {"flag": "🔴", "color": "#d30a14"},
    "Juventude": {"flag": "🟢", "color": "#008a4a"},
    "Criciúma": {"flag": "🟡", "color": "#ffcc00"},
    "Mirassol": {"flag": "🟡", "color": "#ffdd00"},
    "Chapecoense": {"flag": "🟢", "color": "#006633"},
    "Coritiba": {"flag": "🟢", "color": "#005b36"},
    "Atlético Goianiense": {"flag": "🔴", "color": "#dd0000"},
    "Cuiabá": {"flag": "🔰", "color": "#007a33"},
    "Goiás": {"flag": "🟢", "color": "#005c3b"},
    "Ceará": {"flag": "⚫", "color": "#000000"},
    "América Mineiro": {"flag": "🟢", "color": "#008a4f"},
    "Sport": {"flag": "🔴", "color": "#c41221"},
    "Avaí": {"flag": "🔵", "color": "#0056a3"},
    "Remo": {"flag": "🔵", "color": "#00205b"}
}
...
def run_app():
    ...
    if st.sidebar.button("🔄 Actualizar ESPN y Re-entrenar", type="primary"):
        with st.spinner("Descargando últimos resultados del Brasileirão..."):
            rec.recolectar()
            rec_box.recolectar()
        st.cache_data.clear()
    ...
    st.markdown('<div class="main-title">🇽🇷 Portal de Predicción Brasileirão</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_bra():
                sel = st.session_state.get("sel_fix_bra")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in equipos_activos and v_sel in equipos_activos:
                        st.session_state["sel_a_bra"] = l_sel
                        st.session_state["sel_b_bra"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_bra",
                on_change=_on_fixture_change_bra,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            a = st.selectbox("Equipo Local", equipos_activos, index=equipos_activos.index("Flamengo") if "Flamengo" in equipos_activos else 0, key="sel_a_bra", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            b = st.selectbox("Equipo Visitante", equipos_activos, index=equipos_activos.index("Palmeiras") if "Palmeiras" in equipos_activos else (1 if len(equipos_activos) > 1 else 0), key="sel_b_bra", format_func=lambda t: fmt_opcion(equipos, t))
    ...
    with tab2:
        ...
        df_actual_vis["Escudo"] = df_actual_vis["Selección"].apply(lambda t: logo_url(equipos, t))
        df_actual_vis["Equipo"] = df_actual_vis["Selección"].apply(lambda t: label_tabla(equipos, t))
        df_actual_vis = df_actual_vis[["Escudo", "Equipo", "PTS", "PG", "PE", "PP", "DG", "GF"]]
        ...
        df_proy_visual["Escudo"] = df_proy_visual["Selección"].apply(lambda t: logo_url(equipos, t))
        df_proy_visual["Equipo"] = df_proy_visual["Selección"].apply(lambda t: label_tabla(equipos, t))
    ...
    with tab3:
        ...
        st.markdown("La regularización **LASSO** penaliza coeficientes nulos, identificando las características con mayor peso predictivo para el fútbol brasileño.")
        ...
        with col_g:
            ...
            ax.barh(top_n["Variable"][::-1], top_n["Peso Absoluto Promedio"][::-1], color="#00a859")
            ax.set_title("Top 15 Características Predictoras (Brasileirão)")
            st.pyplot(fig)
        ...
        if df_val is None or len(df_val) == 0:
            ...
            with c_plot:
                if met["n"] >= 3:
                    ...
                    ax.plot(evol["partido"], evol["logloss_acum"], "o-", color="#00a859", label="Modelo")
