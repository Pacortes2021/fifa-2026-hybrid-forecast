...
COORDS_BRAZIL = {
    ...
    "Sport": (-8.0476, -34.8770), "Criciúma": (-28.6775, -49.3697),
    "Mirassol": (-20.8175, -49.5083), "Avaí": (-27.5954, -48.5480),
    "Remo": (-1.4558, -48.5039)
}
...
def get_advanced_features(team, season):
    ...
    return pd.Series({
        ...
        "stadium_capacity": 25000, "avg_attendance": 12000, "stadium_occupation": 0.5
    })
...
class StateTracker:
    ...
    def get_features_for_match(self, local, visita, temporada, fecha=None, reset_season=True):
        ...
        # 3b. Características avanzadas de TM
...
def cargar_y_entrenar():
    ...
    pipe_rf_base = Pipeline([("sc", StandardScaler()), ("rf", RandomForestClassifier(n_estimators=500, max_depth=3, min_samples_split=30, random_state=42, n_jobs=-1))])
    ...
    pipe_rf_final = Pipeline([("sc", StandardScaler()), ("rf", RandomForestClassifier(n_estimators=500, max_depth=3, min_samples_split=30, random_state=42, n_jobs=-1))])
...
def _temporada_actual():
    # En Brasil la temporada es el año calendario
...
def _simular_fixture_vec(M, PREDS, fijos, n_sims):
    ...
    for r in fix.itertuples(index=False):
        ...
        GC[vi] += gl
    DG = GF - GC
    key = (PTS * 1_000_000_000 + PG * 1_000_000 + DG * 1_000 + GF).astype(np.int64)
...
def monte_carlo(M, n_sims=4000, fijos=None, modelo="rf", seed=42):
    ...
    lib_d_c = np.bincount(order[:4].ravel(), minlength=n_eq)
    lib_t_c = np.bincount(order[:6].ravel(), minlength=n_eq)
    sud_c = np.bincount(order[6:12].ravel(), minlength=n_eq)
    des_c = np.bincount(order[-4:].ravel(), minlength=n_eq)
