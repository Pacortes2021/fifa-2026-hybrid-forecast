"""
Aplicación Streamlit para la simulación y predicciones de la Premier League (Inglaterra).
...
TEAM_DETAILS = {
    "Arsenal": {"flag": "🔴⚪", "color": "#ef0107"},
    "Aston Villa": {"flag": "🦁", "color": "#670e36"},
    "AFC Bournemouth": {"flag": "🍒", "color": "#da291c"},
    "Brentford": {"flag": "🐝", "color": "#e30613"},
    "Brighton & Hove Albion": {"flag": "🕊️", "color": "#0057b8"},
    "Chelsea": {"flag": "🔵", "color": "#034694"},
    "Crystal Palace": {"flag": "🦅", "color": "#1b458f"},
    "Everton": {"flag": "🔵⚪", "color": "#003399"},
    "Fulham": {"flag": "⚪⚫", "color": "#000000"},
    "Ipswich Town": {"flag": "🐎", "color": "#0033a0"},
    "Leeds United": {"flag": "🤍", "color": "#ffffff"},
    "Leicester City": {"flag": "🦊", "color": "#003090"},
    "Liverpool": {"flag": "🔴", "color": "#c8102e"},
    "Manchester City": {"flag": "🔵", "color": "#6cabdd"},
    "Manchester United": {"flag": "🔴", "color": "#da291c"},
    "Newcastle United": {"flag": "⚫⚪", "color": "#241f20"},
    "Nottingham Forest": {"flag": "🌲", "color": "#dd0000"},
    "Southampton": {"flag": "⛵", "color": "#d71920"},
    "Tottenham Hotspur": {"flag": "🐓", "color": "#132257"},
    "West Ham United": {"flag": "⚒️", "color": "#7a263a"},
    "Wolverhampton Wanderers": {"flag": "🐺", "color": "#fdb913"},
    "Burnley": {"flag": "🔵⚫", "color": "#6c1d45"},
    "Sheffield United": {"flag": "🔴⚪", "color": "#ee2737"},
    "Luton Town": {"flag": "🧡", "color": "#f78f1e"},
    "Watford": {"flag": "🟡🔴", "color": "#fbee23"},
    "Norwich City": {"flag": "🟢🟡", "color": "#fff200"},
    "Sunderland": {"flag": "⚫⚪", "color": "#eb172b"},
    "Hull City": {"flag": "⚫🟠", "color": "#f5971e"},
    "Coventry City": {"flag": "🟢🔵", "color": "#78b824"},
    "West Bromwich Albion": {"flag": "🟤⚪", "color": "#122f67"}
}
...
def run_app():
    ...
    st.markdown('<div class="main-title">🇬🇧 Portal de Predicción Premier League</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_eng():
                sel = st.session_state.get("sel_fix_eng")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_eng"] = l_sel
                        st.session_state["sel_b_eng"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_eng",
                on_change=_on_fixture_change_eng,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            a = st.selectbox("Equipo Local", opciones, index=opciones.index("Arsenal") if "Arsenal" in opciones else 0, key="sel_a_eng", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index("Liverpool") if "Liverpool" in opciones else (1 if len(opciones) > 1 else 0), key="sel_b_eng", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            partidos_df_eng = pd.read_csv(mo.DATA / "partidos.csv")
            mask_h2h = ((partidos_df_eng["local"] == a) & (partidos_df_eng["visita"] == b)) | \
                       ((partidos_df_eng["local"] == b) & (partidos_df_eng["visita"] == a))
            df_h2h = partidos_df_eng[mask_h2h].copy()
        ...
        if df_val is None or len(df_val) == 0:
            ...
            if "resultado" not in df_val.columns:
                df_val["resultado"] = df_val.apply(
                    lambda r: "Gana local" if r["goles_local"] > r["goles_visita"] else ("Empate" if r["goles_local"] == r["goles_visita"] else "Gana visita"),
                    axis=1
                )
