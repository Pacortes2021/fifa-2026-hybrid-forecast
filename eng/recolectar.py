"""
Recolector de la Premier League de Inglaterra desde la API pública de ESPN (liga 'eng.1').
Convierte la fecha UTC de ESPN a la hora local inglesa ('Europe/London') para coincidir exactamente con los días del calendario real.
"""
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Inglaterra.")
        return
    ...
    print(f"\nGuardado Inglaterra: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Inglaterra: fixture.csv ({len(fixture)} por jugar)")
