...
# Constantes de la Premier League de Inglaterra
DESCIENDEN = 3
CUPOS_COPA = 8  # Champions League (1º a 5º, plaza extra por coeficiente) + Europa League (6º y 7º) + Conference (8º)

SQUAD_VALUES = {
    "Manchester City": 1350.0, "Arsenal": 1250.0, "Liverpool": 1100.0,
    "Chelsea": 1000.0, "Manchester United": 850.0, "Tottenham Hotspur": 780.0,
    "Newcastle United": 700.0, "Aston Villa": 620.0, "Brighton & Hove Albion": 560.0,
    "West Ham United": 480.0, "Nottingham Forest": 450.0, "Crystal Palace": 430.0,
    "Brentford": 420.0, "Fulham": 410.0, "Everton": 400.0,
    "Wolverhampton Wanderers": 380.0, "AFC Bournemouth": 320.0, "Leeds United": 310.0,
    "Leicester City": 290.0, "Southampton": 260.0, "Ipswich Town": 220.0,
    "Coventry City": 180.0, "Sunderland": 170.0, "Hull City": 160.0,
    "Luton Town": 150.0, "Burnley": 150.0, "Sheffield United": 140.0,
    "Watford": 120.0, "Norwich City": 110.0, "West Bromwich Albion": 120.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Premier League (las ligas top tienen más volatilidad)
HOME_ADV = 60.0    # Ventaja de local típica de 60 puntos ELO


COORDS_ENGLAND = {
    "Arsenal": (51.5074, -0.1278), "Chelsea": (51.5074, -0.1278),
    "Tottenham Hotspur": (51.5074, -0.1278), "West Ham United": (51.5074, -0.1278),
    "Fulham": (51.5074, -0.1278), "Brentford": (51.5074, -0.1278),
    "Crystal Palace": (51.5074, -0.1278), "AFC Bournemouth": (50.7192, -1.8808),
    "Manchester City": (53.4808, -2.2426), "Manchester United": (53.4808, -2.2426),
    "Liverpool": (53.4084, -2.9916), "Everton": (53.4084, -2.9916),
    "Newcastle United": (54.9783, -1.6178), "Aston Villa": (52.4862, -1.8904),
    "Wolverhampton Wanderers": (52.5869, -2.1275), "Nottingham Forest": (52.9548, -1.1581),
    "Leicester City": (52.6369, -1.1398), "Leeds United": (53.8008, -1.5491),
    "Sheffield United": (53.3811, -1.4701), "Southampton": (50.9097, -1.4044),
    "Brighton & Hove Albion": (50.8225, -0.1372), "Burnley": (53.7891, -2.2400),
    "Watford": (51.6565, -0.3903), "Norwich City": (52.6309, 1.2974),
    "Luton Town": (51.8787, -0.4200), "Ipswich Town": (52.0567, 1.1482),
    "Sunderland": (54.9061, -1.3813), "Hull City": (53.7676, -0.3274),
    "Coventry City": (52.4068, -1.5197),
    "West Bromwich Albion": (52.5091, -1.9640)
}
...
def get_distance_km(local, visita):
    c_loc = COORDS_ENGLAND.get(local)
    c_vis = COORDS_ENGLAND.get(visita)
...
def cargar_y_entrenar():
    ...
    pipe_rf_base = Pipeline([("scale", StandardScaler()), ("rf", RandomForestClassifier(max_depth=5, n_estimators=500, min_samples_split=30, random_state=42, n_jobs=-1))])
    ...
    pipe_rf = Pipeline([("scale", StandardScaler()), ("rf", RandomForestClassifier(max_depth=5, n_estimators=500, min_samples_split=30, random_state=42, n_jobs=-1))])
...
def _temporada_actual():
    # En la Premier League la temporada va de julio a junio (2026 = temporada 2026/27)
...
def validacion_en_vivo(M, temporada_val=None, modelo_tipo="rf"):
    # Mismo reporte de validación en vivo para la Premier League
    partidos = pd.read_csv(DATA / "partidos.csv", parse_dates=["fecha"]).sort_values("fecha")
