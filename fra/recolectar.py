"""
Recolector de la Ligue 1 de Francia desde la API pública de ESPN (liga "fra.1").
Convierte la fecha UTC de ESPN a la hora local francesa ("Europe/Paris").
"""
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Francia (París)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Francia.")
        return
    ...
    print(f"Guardado Francia: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Francia: fixture.csv ({len(fixture)} por jugar)")
