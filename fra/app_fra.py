"""
Aplicación Streamlit para la simulación y predicciones de la Ligue 1 (Francia).
...
# Insertar el directorio fra en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#002395,#ED2939); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#002395;
    border-bottom:2px solid #e2e8f0; padding-bottom:0.4rem; margin-bottom:0.8rem; }
.sec-title { font-size:1.6rem; font-weight:800; color:#002395; margin:0.8rem 0 0.6rem 0; }
...
# Emojis e identidades visuales de los clubes franceses
TEAM_DETAILS = {
    "Paris Saint-Germain": {"flag": "🔴🔵", "color": "#011F68"},
    "Monaco": {"flag": "🔴⚪", "color": "#E91514"},
    "Marseille": {"flag": "⚪🔵", "color": "#2faee0"},
    "Lyon": {"flag": "🔵🔴", "color": "#002f6c"},
    "Lille": {"flag": "🔴⚪", "color": "#c2051b"},
    "Rennes": {"flag": "🔴⚫", "color": "#ef2f24"},
    "Nice": {"flag": "🔴⚫", "color": "#ef2f24"},
    "Lens": {"flag": "🟡🔴", "color": "#f6b400"},
    "Strasbourg": {"flag": "🔵⚪", "color": "#0000bf"},
    "Brest": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Reims": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Nantes": {"flag": "🟡🟢", "color": "#ffff00"},
    "Toulouse": {"flag": "🟣⚪", "color": "#560080"},
    "Montpellier": {"flag": "🟠🔵", "color": "#011F68"},
    "Saint-Etienne": {"flag": "🟢⚪", "color": "#51cc5f"},
    "Le Havre": {"flag": "🔵⚪", "color": "#011F68"},
    "Lorient": {"flag": "🟠⚫", "color": "#f46100"},
    "Auxerre": {"flag": "⚪🔵", "color": "#004595"},
    "Metz": {"flag": "🟤⚪", "color": "#8C3140"},
    "Angers": {"flag": "⚪⚫", "color": "#1a1a1a"},
    "Clermont": {"flag": "🔴🔵", "color": "#8C3140"},
    "Paris FC": {"flag": "🔵⚪", "color": "#002395"},
    "Troyes": {"flag": "🔵⚪", "color": "#0000bf"},
    "Bordeaux": {"flag": "🔵⚪", "color": "#011F68"},
    "AC Ajaccio": {"flag": "🔴⚪", "color": "#ef2f24"},
    "Le Mans": {"flag": "🔴🟡", "color": "#d62b11"},
    "Nimes": {"flag": "🔴⚪", "color": "#ef2f24"}
}
...
def run_app():
    ...
    st.markdown('<div class="main-title">🇫🇷 Portal de Predicción Ligue 1</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_fra():
                sel = st.session_state.get("sel_fix_fra")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_fra"] = l_sel
                        st.session_state["sel_b_fra"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_fra",
                on_change=_on_fixture_change_fra,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            def_a = "Paris Saint-Germain" if "Paris Saint-Germain" in opciones else opciones[0]
            a = st.selectbox("Equipo Local", opciones, index=opciones.index(def_a), key="sel_a_fra", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            def_b = "Marseille" if "Marseille" in opciones else ("Monaco" if "Monaco" in opciones else opciones[1])
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index(def_b), key="sel_b_fra", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            with st.expander("🔬 Ver Radiografía Táctica y Métricas Comparativas", expanded=True):
                ...
                st.markdown(
                    ...
                    f"<span style='color:#002395;'>🏠 {a}</span>"
                    f"<span style='color:#6b7280;font-size:.8rem;'>Variable</span>"
                    f"<span style='color:#ED2939;'>✈️ {b}</span></div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with col_g:
            ...
            ax.barh(top_n["Variable"][::-1], top_n[col_val_name][::-1], color="#002395")
        ...
        if df_val is None or len(df_val) == 0:
            ...
            with c_plot:
                if met["n"] >= 3:
                    ...
                    ax.plot(evol["partidos"], evol["logloss"], "o-", color="#002395", label="Modelo (acumulado)")
                    ax.axhline(met["logloss_base"], color="#ED2939", ls="--", lw=1.5, label="Baseline")
