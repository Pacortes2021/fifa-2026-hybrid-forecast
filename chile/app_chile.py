...
def run_app():
    ...
    with tab1:
        ...
        with c1:
            a = st.selectbox("Equipo Local", equipos_activos, index=equipos_activos.index("Colo Colo") if "Colo Colo" in equipos_activos else 0, key="sel_a", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style=\"text-align:center;margin-top:0.2rem;\">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            b = st.selectbox("Equipo Visitante", equipos_activos, index=equipos_activos.index("Universidad de Chile") if "Universidad de Chile" in equipos_activos else 0, key="sel_b", format_func=lambda t: fmt_opcion(equipos, t))
