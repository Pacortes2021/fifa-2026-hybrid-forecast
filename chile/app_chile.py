...
def run_app():
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_chile():
                sel = st.session_state.get("sel_fix_chile")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in equipos_activos and v_sel in equipos_activos:
                        st.session_state["sel_a_chile"] = l_sel
                        st.session_state["sel_b_chile"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_chile",
                on_change=_on_fixture_change_chile,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            a = st.selectbox("Equipo Local", equipos_activos, index=equipos_activos.index("Colo Colo") if "Colo Colo" in equipos_activos else 0, key="sel_a_chile", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            b = st.selectbox("Equipo Visitante", equipos_activos, index=equipos_activos.index("Universidad de Chile") if "Universidad de Chile" in equipos_activos else (1 if len(equipos_activos) > 1 else 0), key="sel_b_chile", format_func=lambda t: fmt_opcion(equipos, t))
    ...
    with tab2:
        ...
        df_actual_vis = df_actual_vis[["Escudo", "Equipo", "PTS", "DG", "PG", "PE", "PP", "GF"]]
