"""
Recolector de la Serie A de Italia desde la API pública de ESPN (liga "ita.1").
Convierte la fecha UTC de ESPN a la hora local italiana ("Europe/Rome") para coincidir con los días del calendario real.
"""
...
TEAMS_URL = "https://site.api.espn.com/apis/site/v2/sports/soccer/ita.1/teams"
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Italia (Roma)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Rome").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Italia.")
        return
    ...
    print(f"Guardado Italia: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Italia: fixture.csv ({len(fixture)} por jugar)")
