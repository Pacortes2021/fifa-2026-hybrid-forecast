"""
Recolector de equipos de la Swedish Allsvenskan (Suecia) desde ESPN.
...
def recolectar():
    ...
    # 1. Endpoint /teams
    url_teams = "https://site.api.espn.com/apis/site/v2/sports/soccer/swe.1/teams"
    try:
        ...
        print(f"Error consultando /teams swe.1: {ex}")

    # 2. Escanear partidos para capturar equipos descendidos/históricos
    for anio in range(2021, 2027):
        url = f"https://site.api.espn.com/apis/site/v2/sports/soccer/swe.1/scoreboard?dates={anio}0101-{anio}1231&limit=400"
        try:
    ...
    print(f"Guardado {out_p} ({len(df)} equipos suecos)")
    return df
