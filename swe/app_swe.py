"""
Aplicación Streamlit para la simulación y predicciones de la Swedish Allsvenskan (Suecia).
...
# Insertar el directorio swe en el path para asegurar la importación del motor
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import motor as mo
import recolectar as rec
import recolectar_boxscore as rec_box

# Estilizado CSS Premium con paleta de colores de Suecia (Azul Nórdico, Amarillo Oro, Blanco)
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&display=swap');
html, body, [class*="css"] { font-family: 'Outfit', sans-serif; }
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#005293,#FECC00,#1A1A1A); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#005293;
...
# Emojis e identidades visuales de los clubes suecos
TEAM_DETAILS = {
    "Malmo FF": {"flag": "🔵⚪", "color": "#5699eb"},
    "Djurgarden": {"flag": "🔵⚪", "color": "#64a6e1"},
    "Hammarby": {"flag": "🟢⚪", "color": "#007a43"},
    "AIK": {"flag": "⚫🟡", "color": "#c9ad00"},
    "BK Hacken": {"flag": "🟡⚫", "color": "#f7ee09"},
    "IF Elfsborg": {"flag": "🟡⚫", "color": "#ffef32"},
    "IFK Goteborg": {"flag": "🔵⚪", "color": "#214a99"},
    "IFK Norrkoping": {"flag": "⚪🔵", "color": "#00539f"},
    "Mjallby AIF": {"flag": "🟡⚫", "color": "#ffd400"},
    "IK Sirius": {"flag": "🔵⚫", "color": "#d62612"},
    "GAIS": {"flag": "🟢⚫", "color": "#009d58"},
    "Brommapojkarna": {"flag": "🔴⚫", "color": "#df1e1e"},
    "Kalmar FF": {"flag": "🔴⚪", "color": "#db1b30"},
    "Halmstads BK": {"flag": "🔵⚪", "color": "#0058a2"},
    "IFK Varnamo": {"flag": "⚪🔵", "color": "#005a9f"},
    "Vasteras SK": {"flag": "🟢⚪", "color": "#007948"},
    "Degerfors IF": {"flag": "🔴⚪", "color": "#c60000"},
    "Helsingborgs IF": {"flag": "🔴🔵", "color": "#c8102e"},
    "Varbergs BoIS": {"flag": "🟢⚫", "color": "#00843d"},
    "Orebro SK": {"flag": "⚪⚫", "color": "#000000"},
    "Osters IF": {"flag": "🔴🔵", "color": "#ffd700"},
    "GIF Sundsvall": {"flag": "🔵⚪", "color": "#005293"},
    "Ostersunds FK": {"flag": "🔴⚫", "color": "#c8102e"},
    "Orgryte IS": {"flag": "🔴🔵", "color": "#c8102e"}
}
...
def run_app():
    ...
    M = get_motor()
    if M is None:
        st.error("No se encontraron datos históricos de la Swedish Allsvenskan. Ejecuta la recolección primero.")
        return

    tracker = M["tracker"]
    equipos_data = M.get("equipos", {})

    st.markdown('<div class="main-title">🇸🇪 Swedish Allsvenskan Predictor</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fixture_disp.empty:
            st.markdown("#### ⚡ Próximos Partidos Oficiales de Allsvenskan")
    ...
    with tab2:
        ...
        with col_t2:
            ...
            df_mc = simular_liga(M, "sim_key_swe", modelo_tipo)
        ...
        st.info("ℹ️ **Reglas de Allsvenskan**: 16 clubes disputan 30 fechas en formato todos contra todos a ida y vuelta. El campeón clasifica a la ronda previa de la UEFA Champions League; 2º, 3º y el campeón de la Svenska Cupen obtienen cupos para la UEFA Conference League. Los puestos 15º y 16º descienden directamente a la Superettan, mientras el 14º disputa el playoff de permanencia.")
    ...
    with tab3:
        ...
        if rf_step and hasattr(rf_step, "feature_importances_"):
            ...
            ax.barh(df_imp["Variable"][::-1], df_imp["Importancia"][::-1], color="#005293")
