...
DESCIENDEN = 2  # Descienden 2 clubes (puestos 15 y 16) directamente a Superettan
CUPOS_COPA = 4  # 1 a Champions League, 2-4 a Conference League / Europa League

SQUAD_VALUES = {
    "Malmo FF": 45.0, "Djurgarden": 28.0, "Hammarby": 26.0, "AIK": 24.0,
    "BK Hacken": 22.0, "IF Elfsborg": 20.0, "IFK Goteborg": 18.0, "IFK Norrkoping": 14.0,
    "Mjallby AIF": 12.0, "IK Sirius": 12.0, "GAIS": 11.0, "Brommapojkarna": 10.0,
    "Kalmar FF": 10.0, "Halmstads BK": 9.0, "IFK Varnamo": 8.0, "Vasteras SK": 8.0,
    "Degerfors IF": 7.0, "Helsingborgs IF": 7.0, "Varbergs BoIS": 6.0, "Orebro SK": 6.0,
    "Osters IF": 6.0, "GIF Sundsvall": 5.0, "Ostersunds FK": 5.0, "Orgryte IS": 5.0
}
...
K_LIGA = 35.0      # ELO K-factor para Allsvenskan
HOME_ADV = 50.0    # Ventaja de localía estándar en Suecia

COORDS_SWEDEN = {
    "Malmo FF": (55.5836, 12.9886),
    "Djurgarden": (59.2906, 18.0839),
    "Hammarby": (59.2906, 18.0839),
    "AIK": (59.3728, 18.0003),
    "BK Hacken": (57.7214, 11.9328),
    "IF Elfsborg": (57.7347, 12.9372),
    "IFK Goteborg": (57.7058, 11.9797),
    "IFK Norrkoping": (58.5847, 16.1728),
    "Mjallby AIF": (56.0125, 14.7003),
    "IK Sirius": (59.8497, 17.6472),
    "GAIS": (57.7058, 11.9797),
    "Brommapojkarna": (59.3639, 17.8683),
    "Kalmar FF": (56.6853, 16.3197),
    "Halmstads BK": (56.6839, 12.8617),
    "IFK Varnamo": (57.1764, 14.0536),
    "Vasteras SK": (59.6206, 16.5336),
    "Degerfors IF": (59.2392, 14.4372),
    "Helsingborgs IF": (56.0464, 12.7086),
    "Varbergs BoIS": (57.1008, 12.2614),
    "Orebro SK": (59.2736, 15.2289),
    "Osters IF": (56.8803, 14.7761),
    "GIF Sundsvall": (62.3942, 17.3117),
    "Ostersunds FK": (63.1931, 14.6569),
    "Orgryte IS": (57.7058, 11.9797)
}

ALTITUDES_SWEDEN = {
    "Malmo FF": 15, "Djurgarden": 25, "Hammarby": 25, "AIK": 30,
    "BK Hacken": 10, "IF Elfsborg": 145, "IFK Goteborg": 12, "IFK Norrkoping": 35,
    "Mjallby AIF": 5, "IK Sirius": 20, "GAIS": 12, "Brommapojkarna": 25,
    "Kalmar FF": 10, "Halmstads BK": 15, "IFK Varnamo": 150, "Vasteras SK": 30,
    "Degerfors IF": 110, "Helsingborgs IF": 35, "Varbergs BoIS": 20, "Orebro SK": 30,
    "Osters IF": 165, "GIF Sundsvall": 15, "Ostersunds FK": 360, "Orgryte IS": 12
}
...
def get_distance_km(local, visita):
    if local in COORDS_SWEDEN and visita in COORDS_SWEDEN:
        c1 = COORDS_SWEDEN[local]; c2 = COORDS_SWEDEN[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 280.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_SWEDEN.get(local, 30)
    av = ALTITUDES_SWEDEN.get(visita, 30)
    return float(al - av)
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 25.2, "foreigners": 8.0, "pct_foreigners": 0.30,
        "squad_value": float(SQUAD_VALUES.get(equipo, 12.0)), "stadium_capacity": 15000.0,
        "avg_attendance": 9000.0, "stadium_occupation": 0.65
    }
...
def cargar(force_retrain=False):
    ...
    cache_path = DATA.parent / "modelo_swe.pkl"
    ...
    if cache_path.exists() and not force_retrain:
        try:
            ...
            if saved.get("key") == key_actual:
                print("Motor Suecia cargado desde cache de disco")
                _MOTOR_CACHE = saved["motor"]
                return _MOTOR_CACHE
        except Exception as ex:
            print(f"No se pudo cargar cache de modelo sueco: {ex}")

    print("Entrenando motor Suecia (Allsvenskan)...")
    ...
    print(f"Metricas SWE Test>=2025: LASSO={metricas['lasso']} RF={metricas['rf']} Stacking={metricas['stacking']}")
...
def _temporada_actual():
    # En Suecia la liga se juega en año calendario (primavera - otoño)
