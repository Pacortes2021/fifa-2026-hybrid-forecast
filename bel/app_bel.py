"""
Aplicación Streamlit para la simulación y predicciones de la Jupiler Pro League (Bélgica).
...
# Insertar el directorio bel en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#000000,#C60000,#FFCD00); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#C60000;
...
# Emojis e identidades visuales de los clubes belgas
TEAM_DETAILS = {
    "Club Brugge": {"flag": "🔵⚫", "color": "#003399"},
    "Anderlecht": {"flag": "🟣⚪", "color": "#55237b"},
    "Genk": {"flag": "🔵⚪", "color": "#0055a5"},
    "Union Saint-Gilloise": {"flag": "🟡🔵", "color": "#f7d117"},
    "Gent": {"flag": "🔵⚪", "color": "#004b9b"},
    "Antwerp": {"flag": "🔴⚪", "color": "#d4002a"},
    "Cercle Brugge": {"flag": "🟢⚫", "color": "#008000"},
    "Standard Liège": {"flag": "🔴⚪", "color": "#d4002a"},
    "KV Mechelen": {"flag": "🟡🔴", "color": "#f6b400"},
    "KVC Westerlo": {"flag": "🟡🔵", "color": "#f7d117"},
    "Sint-Truiden": {"flag": "🟡🔵", "color": "#f7d117"},
    "Charleroi": {"flag": "⚫⚪", "color": "#000000"},
    "OH Leuven": {"flag": "⚪🔴", "color": "#ffffff"},
    "KV Kortrijk": {"flag": "🔴⚪", "color": "#d4002a"},
    "Dender": {"flag": "🔵⚫", "color": "#003399"},
    "Beerschot": {"flag": "🟣⚪", "color": "#55237b"},
    "Eupen": {"flag": "⚫⚪", "color": "#000000"},
    "RWDM": {"flag": "🔴⚫", "color": "#d4002a"},
    "Zulte-Waregem": {"flag": "🔴🟢", "color": "#d4002a"},
    "KV Oostende": {"flag": "🔴🟡🟢", "color": "#d4002a"},
    "Seraing": {"flag": "🔴⚫", "color": "#d4002a"},
    "Waasland-Beveren": {"flag": "🟡🔵", "color": "#f7d117"},
    "Lommel SK": {"flag": "🟢⚪", "color": "#008000"},
    "Mouscron": {"flag": "🔴⚪", "color": "#d4002a"},
    "RAAL La Louvière": {"flag": "🟢⚪", "color": "#008000"}
}
...
def run_app():
    ...
    st.markdown('<div class="main-title">🇧🇪 Portal de Predicción Jupiler Pro League</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="main-subtitle">Modelo activo: <b>{nombre_modelo}</b> — LASSO + RF + XGBoost + Simulación de Título, Champions y Descenso</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fix_rec.empty:
            ...
            def _on_fixture_change_bel():
                sel = st.session_state.get("sel_fix_bel")
                if sel and sel in fix_map:
                    l_sel, v_sel = fix_map[sel]
                    if l_sel in opciones and v_sel in opciones:
                        st.session_state["sel_a_bel"] = l_sel
                        st.session_state["sel_b_bel"] = v_sel

            st.selectbox(
                "⚡ Cargar Partido de la Próxima Fecha:",
                opciones_fixture,
                key="sel_fix_bel",
                on_change=_on_fixture_change_bel,
                help="Elige un partido oficial programado para cargar ambos equipos automáticamente."
            )

        c1, cvs, c2 = st.columns([5, 1, 5])
        with c1:
            def_a = "Club Brugge" if "Club Brugge" in opciones else opciones[0]
            a = st.selectbox("Equipo Local", opciones, index=opciones.index(def_a), key="sel_a_bel", format_func=lambda t: fmt_opcion(equipos, t))
            st.markdown(f'<div style="text-align:center;margin-top:0.2rem;">{logo_html(equipos, a, 64)}</div>', unsafe_allow_html=True)
        with cvs:
            st.markdown('<div class="vs-text">VS</div>', unsafe_allow_html=True)
        with c2:
            def_b = "Anderlecht" if "Anderlecht" in opciones else ("Union Saint-Gilloise" if "Union Saint-Gilloise" in opciones else opciones[1])
            b = st.selectbox("Equipo Visitante", opciones, index=opciones.index(def_b), key="sel_b_bel", format_func=lambda t: fmt_opcion(equipos, t))
        ...
        if a == b:
            ...
            if p[0] > 0.55 and (la - lb) > 1.0:
                st.info(f"💡 **Ventaja de Localía Marcada**: {a} presenta una probabilidad sólida (>55%) y una expectativa neta superior a +1.0 gol frente a {b}.")
            elif p[2] > 0.50:
                st.warning(f"🚨 **Alerta de Golpe Visitante**: {b} posee perfil favorable para imponerse en cancha ajena ({p[2]:.1%}).")

            # Tarjetas de Probabilidades
            c_p1, c_px, c_p2 = st.columns(3)
            with c_p1:
                st.markdown('<div class="card-title">Victoria Local</div>', unsafe_allow_html=True)
                st.metric(label=f"🏠 {a}", value=f"{p[0]:.1%}", delta=f"Cuota: {mo.cuota(p[0]):.2f}")
            with c_px:
                st.markdown('<div class="card-title">Empate</div>', unsafe_allow_html=True)
                st.metric(label="🤝 Empate", value=f"{p[1]:.1%}", delta=f"Cuota: {mo.cuota(p[1]):.2f}")
            with c_p2:
                st.markdown('<div class="card-title">Victoria Visitante</div>', unsafe_allow_html=True)
                st.metric(label=f"✈️ {b}", value=f"{p[2]:.1%}", delta=f"Cuota: {mo.cuota(p[2]):.2f}")

            # Métricas de goles esperados
            cg1, cg2 = st.columns(2)
            cg1.metric(f"⚽ xG Estimado {a}", f"{la:.2f} goles")
            cg2.metric(f"⚽ xG Estimado {b}", f"{lb:.2f} goles")

            # Comparativa de Métricas Clave
            with st.expander("🔬 Ver Radiografía Táctica y Métricas Comparativas", expanded=True):
                fa_d = mo.get_advanced_features(a, 2026)
                fb_d = mo.get_advanced_features(b, 2026)
                elo_a  = tracker.elos[a]
                elo_b  = tracker.elos[b]
                ...
                form_a = np.mean(list(tracker.recent_results[a])[-5:]) if tracker.recent_results[a] else 0.333
                form_b = np.mean(list(tracker.recent_results[b])[-5:]) if tracker.recent_results[b] else 0.333
                gfa    = np.mean(list(tracker.recent_gf[a])[-5:]) if tracker.recent_gf[a] else 1.0
                gfb    = np.mean(list(tracker.recent_gf[b])[-5:]) if tracker.recent_gf[b] else 1.0
                gaa    = np.mean(list(tracker.recent_ga[a])[-5:]) if tracker.recent_ga[a] else 1.0
                gab    = np.mean(list(tracker.recent_ga[b])[-5:]) if tracker.recent_ga[b] else 1.0
                ...
                st.markdown(
                    ...
                    f"<span style='color:#C60000;'>🏠 {a}</span>"
                    f"<span style='color:#6b7280;font-size:.8rem;'>Variable</span>"
                    f"<span style='color:#1a1a1a;'>✈️ {b}</span></div>", unsafe_allow_html=True)
            ...
            st.markdown('<div class="sec-title">Historial Directo (Head-to-Head)</div>', unsafe_allow_html=True)
            partidos_df = pd.read_csv(mo.DATA / "partidos.csv", parse_dates=["fecha"]).sort_values("fecha", ascending=False)
            h2h = partidos_df[
                ((partidos_df["local"] == a) & (partidos_df["visita"] == b)) |
                ((partidos_df["local"] == b) & (partidos_df["visita"] == a))
            ].copy()

            if h2h.empty:
                st.info(f"No hay registros oficiales de enfrentamientos previos entre {a} y {b} en la base de datos.")
            else:
                victorias_a = len(h2h[((h2h["local"] == a) & (h2h["goles_local"] > h2h["goles_visita"])) | ((h2h["visita"] == a) & (h2h["goles_visita"] > h2h["goles_local"]))])
                victorias_b = len(h2h[((h2h["local"] == b) & (h2h["goles_local"] > h2h["goles_visita"])) | ((h2h["visita"] == b) & (h2h["goles_visita"] > h2h["goles_local"]))])
                empates = len(h2h[h2h["goles_local"] == h2h["goles_visita"]])
                total_h2h = len(h2h)

                c_h1, c_h2, c_h3, c_h4 = st.columns(4)
                c_h1.metric("Partidos Registrados", total_h2h)
                c_h2.metric(f"Victorias {a}", f"{victorias_a} ({victorias_a/total_h2h:.0%})")
                c_h3.metric("Empates", f"{empates} ({empates/total_h2h:.0%})")
                c_h4.metric(f"Victorias {b}", f"{victorias_b} ({victorias_b/total_h2h:.0%})")
    ...
    with tab2:
        ...
        with col_proj:
            st.markdown('<div class="card-title">Proyección de Temporada (Monte Carlo)</div>', unsafe_allow_html=True)
            st.caption("Simula el resto de la temporada a partir del fixture oficial con 50.000 iteraciones.")
            st.dataframe(
                df_proy_visual.style.format({
                    ...
                    "🇪🇺 P(Copas)": "{:.1%}",
                    "🔻 P(Descenso)": "{:.1%}"
                }).background_gradient(subset=["🏆 P(Campeón)"], cmap="Greens")
    ...
    with tab3:
        ...
        with col_g:
            ...
            ax.barh(top_n["Variable"][::-1], top_n[col_val_name][::-1], color="#C60000")
        ...
        if df_val is None or len(df_val) == 0:
            ...
            with c_plot:
                if met["n"] >= 3:
                    ...
                    ax.plot(evol["partidos"], evol["logloss"], "o-", color="#C60000", label="Modelo (acumulado)")
                    ax.axhline(met["logloss_base"], color="#000000", ls="--", lw=1.5, label="Baseline")
