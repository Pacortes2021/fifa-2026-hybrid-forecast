"""
Recolector de equipos de la Jupiler Pro League (Bélgica) desde ESPN.
...
def recolectar():
    ...
    # 1. Endpoint /teams
    url_teams = "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/teams"
    try:
        ...
        print(f"Error consultando /teams bel.1: {ex}")

    # 2. Escanear partidos para capturar equipos descendidos/históricos
    for anio in range(2021, 2027):
        url = f"https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/scoreboard?dates={anio}0101-{anio}1231&limit=400"
        try:
    ...
    print(f"Guardado {out_p} ({len(df)} equipos belgas)")
    return df
