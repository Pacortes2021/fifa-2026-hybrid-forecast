"""
Recolector de la Jupiler Pro League de Bélgica desde la API pública de ESPN (liga "bel.1").
Convierte la fecha UTC de ESPN a la hora local belga ("Europe/Brussels").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/scoreboard"
...
NORM_MAP = {
    ...
    "KV Red Star Waasland-Beveren": "Waasland-Beveren",
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Bélgica (Bruselas)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Brussels").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Bélgica.")
        return
    ...
    print(f"Guardado Bélgica: partidos.csv ({len(jugados)} jugados)")
    print(f"Guardado Bélgica: fixture.csv ({len(fixture)} por jugar)")
