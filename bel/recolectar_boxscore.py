"""
Recolecta el BOX SCORE detallado de cada partido de la Jupiler Pro League desde ESPN:
...
SB = "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/scoreboard"
SUM = "https://site.api.espn.com/apis/site/v2/sports/soccer/bel.1/summary"
...
def recolectar():
    ...
    print(f"Descargando {len(a_procesar)} partidos nuevos en paralelo con 20 workers...")
    ...
    if nuevos_resultados:
        ...
        if len(ya):
            df_final = pd.concat([ya, df_nuevos], ignore_index=True).drop_duplicates(subset=["event_id"])
        else:
            df_final = df_nuevos
        df_final.to_csv(path, index=False)
        print(f"Completado: {len(df_nuevos)} nuevos partidos añadidos a box_score.csv. Total registros: {len(df_final)}.")
        return df_final
    else:
        print("No se obtuvieron nuevos registros.")
        return ya
