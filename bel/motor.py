...
DESCIENDEN = 3
CUPOS_COPA = 5  # Champions (1-2), Europa League (3), Conference (4-5)

SQUAD_VALUES = {
    "Club Brugge": 140.0, "Anderlecht": 110.0, "Genk": 100.0,
    "Union Saint-Gilloise": 90.0, "Gent": 85.0, "Antwerp": 75.0,
    "Cercle Brugge": 55.0, "Standard Liège": 50.0, "KV Mechelen": 40.0,
    "KVC Westerlo": 38.0, "Sint-Truiden": 35.0, "Charleroi": 35.0,
    "OH Leuven": 32.0, "KV Kortrijk": 22.0, "Dender": 18.0,
    "Beerschot": 15.0, "Eupen": 14.0, "RWDM": 14.0,
    "Zulte-Waregem": 12.0, "KV Oostende": 10.0, "Seraing": 8.0,
    "Waasland-Beveren": 8.0, "Lommel SK": 8.0, "Mouscron": 6.0,
    "RAAL La Louvière": 6.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Jupiler Pro League
HOME_ADV = 50.0    # Ventaja de local típica en Bélgica

COORDS_BELGIUM = {
    "Club Brugge": (51.1931, 3.1803), "Anderlecht": (50.8342, 4.2983),
    "Genk": (50.9908, 5.5342), "Union Saint-Gilloise": (50.8164, 4.3314),
    "Gent": (51.0161, 3.7344), "Antwerp": (51.2294, 4.4719),
    "Cercle Brugge": (51.1931, 3.1803), "Standard Liège": (50.6097, 5.5436),
    "KV Mechelen": (51.0394, 4.4828), "KVC Westerlo": (51.0911, 4.9228),
    "Sint-Truiden": (50.8164, 5.1683), "Charleroi": (50.4161, 4.4550),
    "OH Leuven": (50.8672, 4.6950), "KV Kortrijk": (50.8331, 3.2508),
    "Dender": (50.8931, 4.0736), "Beerschot": (51.1925, 4.3831),
    "Eupen": (50.6275, 6.0461), "RWDM": (50.8569, 4.3106),
    "Zulte-Waregem": (50.8833, 3.4278), "KV Oostende": (51.2167, 2.8944),
    "Seraing": (50.5967, 5.5139), "Waasland-Beveren": (51.2125, 4.2508),
    "Lommel SK": (51.2333, 5.3083), "Mouscron": (50.7411, 3.2106),
    "RAAL La Louvière": (50.4800, 4.1867)
}

ALTITUDES_BELGIUM = {
    "Club Brugge": 12, "Anderlecht": 35, "Genk": 70, "Union Saint-Gilloise": 65,
    "Gent": 10, "Antwerp": 10, "Cercle Brugge": 12, "Standard Liège": 65,
    "KV Mechelen": 10, "KVC Westerlo": 20, "Sint-Truiden": 55, "Charleroi": 120,
    "OH Leuven": 30, "KV Kortrijk": 20, "Dender": 20, "Beerschot": 10,
    "Eupen": 270, "RWDM": 40, "Zulte-Waregem": 15, "KV Oostende": 5,
    "Seraing": 80, "Waasland-Beveren": 10, "Lommel SK": 45, "Mouscron": 40,
    "RAAL La Louvière": 120
}
...
def get_distance_km(local, visita):
    if local in COORDS_BELGIUM and visita in COORDS_BELGIUM:
        c1 = COORDS_BELGIUM[local]; c2 = COORDS_BELGIUM[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 95.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_BELGIUM.get(local, 30)
    av = ALTITUDES_BELGIUM.get(visita, 30)
    return float(al - av)
...
def get_squad_value(equipo, temporada):
    ...
    return float(SQUAD_VALUES.get(equipo, 40.0))
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 25.2, "foreigners": 16.0, "pct_foreigners": 0.58,
        "squad_value": float(SQUAD_VALUES.get(equipo, 40.0)), "stadium_capacity": 18000.0,
        "avg_attendance": 12000.0, "stadium_occupation": 0.68
    }
...
class StateTracker:
    ...
    def get_features_for_match(self, local, visita, temporada, fecha=None, reset_season=False):
        ...
        for s in STATS:
            hl = [m.get(f"local_{s}", 0.0) for m in self.history_home[local][-5:] if m]
            hv = [m.get(f"visita_{s}", 0.0) for m in self.history_away[visita][-5:] if m]
            ml_val = float(np.mean(hl)) if len(hl) >= 2 else 0.0
            mv_val = float(np.mean(hv)) if len(hv) >= 2 else 0.0
            feats[f"stat_{s}_diff"] = ml_val - mv_val

        return feats

    def registrar_partido(self, local, visita, gl, gv, stats_l=None, stats_v=None, fecha=None):
        el = self.elos[local]
        ev = self.elos[visita]
        dr = (el + HOME_ADV) - ev
        ...
        k = K_LIGA
        self.elos[local] += k * (w - we)
        self.elos[visita] += k * ((1.0 - w) - (1.0 - we))

        self.pi_tracker.registrar_partido(local, visita, gl, gv)
        self.h2h_goles[(local, visita)] += (gl - gv)
        self.h2h_goles[(visita, local)] += (gv - gl)

        p_local = 3 if gl > gv else (1 if gl == gv else 0)
        p_visita = 3 if gv > gl else (1 if gl == gv else 0)
        self.season_pts[local] += p_local
        self.season_pts[visita] += p_visita
        ...
        m_dict_l = {f"local_{s}": (stats_l.get(s, 0.0) if stats_l else 0.0) for s in STATS}
        m_dict_v = {f"visita_{s}": (stats_v.get(s, 0.0) if stats_v else 0.0) for s in STATS}
        self.history_home[local].append(m_dict_l)
        self.history_away[visita].append(m_dict_v)
        ...
        self.recent_gf[local].append(gl);       self.recent_gf[visita].append(gv)
        self.recent_ga[local].append(gv);       self.recent_ga[visita].append(gl)
...
def _cache_key():
    import hashlib
    fp = hashlib.sha256()
    for fname in ["partidos.csv", "box_score.csv", "equipos.csv", "advanced_features_historical.csv"]:
        f = DATA / fname
        if f.exists():
            fp.update(f.name.encode())
            fp.update(str(f.stat().st_mtime).encode())
...
def cargar_y_entrenar():
    ...
    print(f"Metricas BEL Test>=2025: LASSO={m_l} RF={m_r} Stacking={m_s}")
...
def cargar():
    ...
    print("Entrenando modelo de Jupiler Pro League (Stacking + Poisson)... esto toma ~10s")
    M = cargar_y_entrenar()
    try:
