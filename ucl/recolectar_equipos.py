"""
Recolector de equipos de la UEFA Champions League desde ESPN.
...
def recolectar():
    ...
    # 1. Endpoint /teams oficial de Champions League
    url_teams = "https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/teams"
    try:
        ...
        if r.status_code == 200:
            ...
            for t in teams:
                ...
                equipos_map[norm_nm] = {
                    ...
                    "alternate_color": team.get("alternateColor", "FECC00"),
                    "logo": logo or f"https://a.espncdn.com/i/teamlogos/soccer/500-dark/{team['id']}.png",
                    "is_active": team.get("isActive", True)
                }
    except Exception as ex:
        print(f"Error consultando /teams uefa.champions: {ex}")

    # 2. Escanear partidos para capturar todos los clubes que hayan jugado Champions desde 2021
    for anio in range(2021, 2028):
        for ini, fin in ((f"{anio}0101", f"{anio}0630"), (f"{anio}0701", f"{anio}1231")):
            url = f"https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/scoreboard?dates={ini}-{fin}&limit=400"
            try:
                    ...
                    for c in e.get("competitions", [{}])[0].get("competitors", []):
                        ...
                        if norm_nm not in equipos_map:
                            ...
                            equipos_map[norm_nm] = {
                                ...
                                "slug": tm.get("slug", ""),
                                "color": tm.get("color", "001438"),
                                "alternate_color": tm.get("alternateColor", "FECC00"),
                                "logo": logo,
                                "is_active": True
                            }
            except Exception:
    ...
    print(f"Guardado {out_p} ({len(df)} clubes de Champions League)")
    return df
