"""
Recolecta el BOX SCORE detallado de cada partido de la UEFA Champions League desde ESPN:
Recupera las estadísticas completas de posesión, tiros, córners, faltas, etc.
"""
...
SB = "https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/scoreboard"
SUM = "https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/summary"
...
def recolectar():
    ...
    eventos = [ev for a in YEAR_RANGES for ev in _eventos(a)]
    ...
    print(f"Descargando {len(a_procesar)} partidos nuevos de Champions en paralelo con 20 workers...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_ev):
            ...
            count += 1
            if count % 200 == 0:
                print(f"  Progreso: {count}/{len(a_procesar)} partidos procesados...")

    if nuevos_resultados:
        df_nuevos = pd.DataFrame(nuevos_resultados)
        if len(ya) > 0:
            df_final = pd.concat([ya, df_nuevos], ignore_index=True)
        else:
            df_final = df_nuevos
        df_final = df_final.drop_duplicates(subset=["event_id"], keep="last")
        df_final.sort_values("fecha", inplace=True)
        df_final.to_csv(path, index=False)
        print(f"Éxito: {len(df_final)} partidos totales de Champions guardados en {path}")
        return df_final
    else:
        print("No se pudieron obtener nuevos box scores.")
        return ya
