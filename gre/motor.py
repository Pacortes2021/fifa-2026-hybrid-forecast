...
DESCIENDEN = 2  # Descienden 2 clubes (puestos 13 y 14) a Super League 2
CUPOS_COPA = 4  # Champions League (1-2), Europa League (3), Conference League (4)

SQUAD_VALUES = {
    "Olympiacos": 110.0, "Panathinaikos": 95.0, "PAOK": 90.0,
    "AEK Athens": 80.0, "Aris": 35.0, "OFI Crete": 18.0,
    "Asteras Tripoli": 16.0, "Atromitos": 14.0, "Panetolikos": 12.0,
    "Volos NFC": 12.0, "Panserraikos FC": 11.0, "Levadiakos": 11.0,
    "Lamia": 10.0, "Athens Kallithea": 9.0, "Pas Giannina": 8.0,
    "Kifisia": 8.0, "Ionikos FC": 7.0, "Apollon Smyrni": 6.0,
    "Larissa FC": 6.0, "Iraklis": 5.0, "Kalamata": 5.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Greek Super League
HOME_ADV = 55.0    # Fuerte ventaja de localía en Grecia

COORDS_GREECE = {
    "Olympiacos": (37.9464, 23.6644), "Panathinaikos": (37.9872, 23.7542),
    "AEK Athens": (38.0436, 23.7428), "PAOK": (40.6136, 22.9722),
    "Aris": (40.5989, 22.9708), "Asteras Tripoli": (37.5089, 22.3853),
    "OFI Crete": (35.3378, 25.1128), "Atromitos": (38.0142, 23.6917),
    "Lamia": (38.8986, 22.4339), "Volos NFC": (39.3886, 22.9317),
    "Panetolikos": (38.6311, 21.4117), "Panserraikos FC": (41.0928, 23.5583),
    "Levadiakos": (38.4358, 22.8686), "Athens Kallithea": (37.9547, 23.7003),
    "Pas Giannina": (39.6672, 20.8497), "Kifisia": (38.0739, 23.8114),
    "Ionikos FC": (37.9717, 23.6483), "Apollon Smyrni": (38.0261, 23.7439),
    "Larissa FC": (39.6203, 22.4286), "Iraklis": (40.6272, 22.9667),
    "Kalamata": (37.0389, 22.1156)
}

ALTITUDES_GREECE = {
    "Olympiacos": 10, "Panathinaikos": 110, "AEK Athens": 130,
    "PAOK": 35, "Aris": 40, "Asteras Tripoli": 660,
    "OFI Crete": 30, "Atromitos": 50, "Lamia": 180,
    "Volos NFC": 75, "Panetolikos": 90, "Panserraikos FC": 60,
    "Levadiakos": 160, "Athens Kallithea": 25, "Pas Giannina": 480,
    "Kifisia": 280, "Ionikos FC": 35, "Apollon Smyrni": 110,
    "Larissa FC": 80, "Iraklis": 45, "Kalamata": 15
}
...
def get_distance_km(local, visita):
    if local in COORDS_GREECE and visita in COORDS_GREECE:
        c1 = COORDS_GREECE[local]; c2 = COORDS_GREECE[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 200.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_GREECE.get(local, 100)
    av = ALTITUDES_GREECE.get(visita, 100)
    return float(al - av)
...
def get_squad_value(equipo, temporada):
    ...
    return float(SQUAD_VALUES.get(equipo, 20.0))
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 26.5, "foreigners": 15.0, "pct_foreigners": 0.55,
        "squad_value": float(SQUAD_VALUES.get(equipo, 20.0)), "stadium_capacity": 15000.0,
        "avg_attendance": 10000.0, "stadium_occupation": 0.65
    }
...
def cargar():
    ...
    print("Entrenando modelo de Greek Super League (Stacking + Poisson)... esto toma ~10s")
    ...
    for _, row in df_partidos.iterrows():
        ...
        feats = tracker.get_features_for_match(loc, vis, temp, fecha=fecha)
        feats["resultado"] = 0 if ga > gb else (1 if ga == gb else 2)
        ...
        filas_poisson.append({
            "equipo": loc, "rival": vis, "goles": ga, "is_home": 1,
            "elo_diff": feats["elo_diff"], "alt_diff": feats["altitude_diff"],
            "squad_diff": feats["squad_value_diff"]
        })
        filas_poisson.append({
            "equipo": vis, "rival": loc, "goles": gb, "is_home": 0,
            "elo_diff": -feats["elo_diff"], "alt_diff": -feats["altitude_diff"],
            "squad_diff": -feats["squad_value_diff"]
        })

        st_data = box_dict.get(eid, None)
        tracker.registrar_partido(loc, vis, ga, gb, fecha, stats_dict=st_data)

    df_dataset = pd.DataFrame(filas_dataset)

    cols_excluir = {"resultado", "temporada", "fecha", "local", "visita"}
    ...
    print(f"Metricas GRE Test>=2025: LASSO={metricas['lasso']} RF={metricas['rf']} Stacking={metricas['stacking']}")
...
def simular_campeonato(M, n_sims=1000, fijos=None, modelo="stacking", modelo_tipo=None):
    ...
    for eq in eqs:
        res.append({
            "equipo": eq,
            "Selección": eq,
            "Puntos esperados": round(float(tab.loc[tab.equipo == eq, "puntos"].values[0]) + (32 - int(tab.loc[tab.equipo == eq, "pj"].values[0])) * 1.35, 1) if eq in tab["equipo"].values else 40.0,
