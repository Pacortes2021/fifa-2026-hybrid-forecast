"""
Recolector de la Greek Super League (Grecia) desde la API pública de ESPN (liga "gre.1").
Convierte la fecha UTC de ESPN a la hora local griega ("Europe/Athens").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/gre.1/scoreboard"
...
NORM_MAP = {
    ...
    "AEP Kifisia": "Kifisia",
    "Ionikos": "Ionikos FC",
    "Apollon": "Apollon Smyrni",
    "Larissa": "Larissa FC",
    "AEL Larissa": "Larissa FC"
}
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Grecia (Atenas / EET / EEST)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Athens").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Grecia.")
        return
    ...
    print(f"Guardado {out_p} ({len(df_jugados)} partidos jugados de Grecia)")
