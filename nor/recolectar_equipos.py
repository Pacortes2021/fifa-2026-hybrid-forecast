"""
Recolector de equipos de la Norwegian Eliteserien (Noruega) desde ESPN.
...
def recolectar():
    ...
    # 1. Endpoint /teams
    url_teams = "https://site.api.espn.com/apis/site/v2/sports/soccer/nor.1/teams"
    try:
        ...
        if r.status_code == 200:
            ...
            for t in teams:
                ...
                equipos_map[norm_nm] = {
                    ...
                    "slug": team.get("slug", ""),
                    "color": team.get("color", "BA0C2F"),
                    "alternate_color": team.get("alternateColor", "00205B"),
                    "logo": logo or f"https://a.espncdn.com/i/teamlogos/soccer/500-dark/{team['id']}.png",
                    "is_active": team.get("isActive", True)
                }
    except Exception as ex:
        print(f"Error consultando /teams nor.1: {ex}")

    # 2. Escanear partidos para capturar equipos descendidos/históricos
    for anio in range(2021, 2027):
        url = f"https://site.api.espn.com/apis/site/v2/sports/soccer/nor.1/scoreboard?dates={anio}0101-{anio}1231&limit=400"
        try:
                ...
                for c in e.get("competitions", [{}])[0].get("competitors", []):
                    ...
                    if norm_nm not in equipos_map:
                        ...
                        equipos_map[norm_nm] = {
                            ...
                            "slug": tm.get("slug", ""),
                            "color": tm.get("color", "BA0C2F"),
                            "alternate_color": tm.get("alternateColor", "00205B"),
                            "logo": logo,
                            "is_active": True
                        }
        except Exception:
    ...
    print(f"Guardado {out_p} ({len(df)} equipos noruegos)")
    return df
