"""
Recolector de la Süper Lig de Turquía desde la API pública de ESPN (liga "tur.1").
Convierte la fecha UTC de ESPN a la hora local turca ("Europe/Istanbul").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/tur.1/scoreboard"
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Turquía (Estambul)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Istanbul").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Turquía.")
        return
    ...
    print(f"Guardado {out_p} ({len(df_jugados)} partidos jugados de Turquía)")
