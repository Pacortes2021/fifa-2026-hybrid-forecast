"""
Aplicación Streamlit para la simulación y predicciones de la Austrian Bundesliga (Austria).
...
# Insertar el directorio aut en el path para asegurar la importación del motor
...
.main-title { text-align:center; font-size:2.8rem; font-weight:800;
    background:linear-gradient(135deg,#ED2939,#A6101D,#1A1A1A); -webkit-background-clip:text;
...
.card-title { font-size:1.25rem; font-weight:700; color:#ED2939;
...
# Emojis e identidades visuales de los clubes austriacos
TEAM_DETAILS = {
    "RB Salzburg": {"flag": "🔴⚪", "color": "#d82c3a"},
    "SK Sturm Graz": {"flag": "⚫⚪", "color": "#000000"},
    "Rapid Vienna": {"flag": "🟢⚪", "color": "#2b6a36"},
    "LASK Linz": {"flag": "⚫⚪", "color": "#000000"},
    "Austria Vienna": {"flag": "🟣⚪", "color": "#745692"},
    "Wolfsberger": {"flag": "⚫⚪", "color": "#1a1a1a"},
    "TSV Hartberg": {"flag": "🔵⚪", "color": "#00458b"},
    "SK Austria Klagenfurt": {"flag": "🟣⚪", "color": "#800080"},
    "FC Blau-Weiß Linz": {"flag": "🔵⚪", "color": "#163277"},
    "SC Rheindorf Altach": {"flag": "⚫⚪", "color": "#000000"},
    "WSG Swarovski Tirol": {"flag": "🟢⚪", "color": "#008751"},
    "Grazer AK": {"flag": "🔴⚪", "color": "#c60000"},
    "Austria Lustenau": {"flag": "🟢⚪", "color": "#008751"},
    "SV Josko Ried": {"flag": "🟢⚪", "color": "#008751"},
    "FC Admira Wacker Modling": {"flag": "⚫🔴", "color": "#c60000"},
    "St. Pölten": {"flag": "🟡🔵", "color": "#00458b"}
}
...
def run_app():
    ...
    M = get_motor()
    if M is None:
        st.error("No se encontraron datos históricos de la Austrian Bundesliga. Ejecuta la recolección primero.")
        return

    tracker = M["tracker"]
    equipos_data = M.get("equipos", {})

    st.markdown('<div class="main-title">🇦🇹 Austrian Bundesliga Predictor</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        if not fixture_disp.empty:
            st.markdown("#### ⚡ Próximos Partidos Oficiales de Austrian Bundesliga")
    ...
    with tab2:
        ...
        with col_t2:
            ...
            df_mc = simular_liga(M, "sim_key_aut", modelo_tipo)
        ...
        st.info("ℹ️ **Reglas de Austrian Bundesliga**: Puestos 1-2 clasifican a UEFA Champions League (fases previas), 3 a Europa League, 4-5 a Conference League / Playoffs. El 12º (último puesto) desciende directo a 2. Liga.")
    ...
    with tab3:
        ...
        if rf_step and hasattr(rf_step, "feature_importances_"):
            ...
            ax.barh(df_imp["Variable"][::-1], df_imp["Importancia"][::-1], color="#ED2939")
