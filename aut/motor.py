...
DESCIENDEN = 1  # 1 descenso directo (12º de la Qualifikationsgruppe)
CUPOS_COPA = 5  # Champions (1-2), Europa League (3), Conference (4-5)

SQUAD_VALUES = {
    "RB Salzburg": 170.0, "SK Sturm Graz": 65.0, "Rapid Vienna": 35.0,
    "LASK Linz": 35.0, "Austria Vienna": 25.0, "Wolfsberger": 16.0,
    "TSV Hartberg": 14.0, "SK Austria Klagenfurt": 12.0, "SC Rheindorf Altach": 11.0,
    "FC Blau-Weiß Linz": 11.0, "WSG Swarovski Tirol": 10.0, "Grazer AK": 10.0,
    "Austria Lustenau": 8.0, "SV Josko Ried": 8.0, "FC Admira Wacker Modling": 6.0,
    "St. Pölten": 6.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Austrian Bundesliga
HOME_ADV = 50.0    # Ventaja de local típica en Austria

COORDS_AUSTRIA = {
    "RB Salzburg": (47.8153, 12.9986), "SK Sturm Graz": (47.0456, 15.4542),
    "Rapid Vienna": (48.1981, 16.2650), "LASK Linz": (48.2989, 14.2758),
    "Austria Vienna": (48.1594, 16.3886), "Wolfsberger": (46.8458, 14.8389),
    "TSV Hartberg": (47.2800, 15.9681), "SK Austria Klagenfurt": (46.6089, 14.2792),
    "SC Rheindorf Altach": (47.3517, 9.6467), "FC Blau-Weiß Linz": (48.3117, 14.2983),
    "WSG Swarovski Tirol": (47.2567, 11.4117), "Grazer AK": (47.0456, 15.4542),
    "Austria Lustenau": (47.4333, 9.6667), "SV Josko Ried": (48.2100, 13.4883),
    "FC Admira Wacker Modling": (48.0967, 16.3117), "St. Pölten": (48.2239, 15.6528)
}

ALTITUDES_AUSTRIA = {
    "RB Salzburg": 424, "SK Sturm Graz": 350, "Rapid Vienna": 210,
    "LASK Linz": 266, "Austria Vienna": 190, "Wolfsberger": 460,
    "TSV Hartberg": 360, "SK Austria Klagenfurt": 446, "SC Rheindorf Altach": 412,
    "FC Blau-Weiß Linz": 260, "WSG Swarovski Tirol": 580, "Grazer AK": 350,
    "Austria Lustenau": 405, "SV Josko Ried": 433, "FC Admira Wacker Modling": 220,
    "St. Pölten": 267
}
...
def get_distance_km(local, visita):
    if local in COORDS_AUSTRIA and visita in COORDS_AUSTRIA:
        c1 = COORDS_AUSTRIA[local]; c2 = COORDS_AUSTRIA[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 160.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_AUSTRIA.get(local, 350)
    av = ALTITUDES_AUSTRIA.get(visita, 350)
    return float(al - av)
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 25.5, "foreigners": 13.0, "pct_foreigners": 0.45,
        "squad_value": float(SQUAD_VALUES.get(equipo, 20.0)), "stadium_capacity": 15000.0,
        "avg_attendance": 9000.0, "stadium_occupation": 0.60
    }
...
def cargar():
    ...
    print("Entrenando modelo de Austrian Bundesliga (Stacking + Poisson)... esto toma ~10s")
    ...
    print(f"Metricas AUT Test>=2025: LASSO={metricas['lasso']} RF={metricas['rf']} Stacking={metricas['stacking']}")
