"""
Recolector de la Danish Superliga (Dinamarca) desde la API pública de ESPN (liga "den.1").
Convierte la fecha UTC de ESPN a la hora local danesa ("Europe/Copenhagen").
"""
...
SCOREBOARD = "https://site.api.espn.com/apis/site/v2/sports/soccer/den.1/scoreboard"
TEMPORADAS = list(range(2021, int(datetime.now().year) + 1))

NORM_MAP = {
    "FC Copenhagen": "F.C. København",
    "FC Kobenhavn": "F.C. København",
...
def _temporada(anio):
    ...
    for e in eventos:
        try:
            ...
            # Convertir hora UTC a la zona horaria local de Dinamarca (Copenhague / CET / CEST)
            fecha_local = pd.to_datetime(e["date"]).tz_convert("Europe/Copenhagen").tz_localize(None)
...
def recolectar():
    ...
    if not todo:
        print("No se pudieron recolectar partidos de Dinamarca.")
        return
    ...
    print(f"Guardado {out_p} ({len(df_jugados)} partidos jugados de Dinamarca)")
