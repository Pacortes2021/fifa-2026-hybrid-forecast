...
DESCIENDEN = 2  # Descienden 2 clubes (puestos 11 y 12) a 1. Division
CUPOS_COPA = 4  # Champions League (1-2), Conference League / Playoff (3-4)

SQUAD_VALUES = {
    "F.C. København": 80.0, "FC Midtjylland": 65.0, "Brøndby IF": 45.0,
    "FC Nordsjælland": 40.0, "AGF": 22.0, "Silkeborg IF": 16.0,
    "Randers FC": 14.0, "Viborg FF": 14.0, "AaB": 12.0,
    "Odense Boldklub": 12.0, "Vejle Boldklub": 10.0, "Lyngby Boldklub": 9.0,
    "Sønderjyske Fodbold": 9.0, "AC Horsens": 7.0, "Hvidovre IF": 5.0,
    "FC Fredericia": 5.0
}
...
K_LIGA = 35.0      # ELO K-factor para la Danish Superliga
HOME_ADV = 50.0    # Ventaja de local típica en Dinamarca

COORDS_DENMARK = {
    "F.C. København": (55.7028, 12.5725),
    "Brøndby IF": (55.6483, 12.4183),
    "FC Midtjylland": (56.1178, 8.9525),
    "FC Nordsjælland": (55.8156, 12.3556),
    "AGF": (56.1317, 10.1972),
    "Silkeborg IF": (56.1836, 9.5744),
    "Randers FC": (56.4678, 10.0194),
    "Viborg FF": (56.4533, 9.4017),
    "AaB": (57.0547, 9.8986),
    "Odense Boldklub": (55.3986, 10.3508),
    "Vejle Boldklub": (55.7139, 9.5606),
    "Lyngby Boldklub": (55.7761, 12.5028),
    "Sønderjyske Fodbold": (55.2575, 9.4886),
    "AC Horsens": (55.8714, 9.8606),
    "Hvidovre IF": (55.6517, 12.4856),
    "FC Fredericia": (55.5786, 9.7347)
}

ALTITUDES_DENMARK = {
    "F.C. København": 12, "Brøndby IF": 18, "FC Midtjylland": 50,
    "FC Nordsjælland": 35, "AGF": 20, "Silkeborg IF": 30,
    "Randers FC": 25, "Viborg FF": 40, "AaB": 10,
    "Odense Boldklub": 15, "Vejle Boldklub": 60, "Lyngby Boldklub": 25,
    "Sønderjyske Fodbold": 30, "AC Horsens": 25, "Hvidovre IF": 10,
    "FC Fredericia": 35
}
...
def get_distance_km(local, visita):
    if local in COORDS_DENMARK and visita in COORDS_DENMARK:
        c1 = COORDS_DENMARK[local]; c2 = COORDS_DENMARK[visita]
        return haversine_km(c1[0], c1[1], c2[0], c2[1])
    return 130.0

def get_altitude_diff(local, visita):
    al = ALTITUDES_DENMARK.get(local, 25)
    av = ALTITUDES_DENMARK.get(visita, 25)
    return float(al - av)
...
def get_squad_value(equipo, temporada):
    ...
    return float(SQUAD_VALUES.get(equipo, 18.0))
...
def get_advanced_features(equipo, temporada):
    ...
    return {
        "squad_size": 28.0, "avg_age": 25.2, "foreigners": 12.0, "pct_foreigners": 0.45,
        "squad_value": float(SQUAD_VALUES.get(equipo, 18.0)), "stadium_capacity": 12000.0,
        "avg_attendance": 8000.0, "stadium_occupation": 0.65
    }
...
def cargar():
    ...
    print("Entrenando modelo de Danish Superliga (Stacking + Poisson)... esto toma ~10s")
    ...
    print(f"Metricas DEN Test>=2025: LASSO={metricas['lasso']} RF={metricas['rf']} Stacking={metricas['stacking']}")
